 ...
 */
var wasmBinaryFile;
wasmBinaryFile = scriptDirectory + "../webassembly/girweb.wasm";
...
/*
 * Copied from GeneralisedIsotonicRegressionPlots.js
 *
 * Should put plotting code in another library
 * so both can use it
 */
var result;
...
function plot2d(plot_id, data) {
    var active_iteration = 0;
    var iterations = { };
    var slider_steps = [];
    var frames = [];

    var x = Array(data.rows);
    var y = Array(data.rows);
    for (row = 0; row < data.rows; row++) {
        x[row] = data.get_x(row, 0);
        y[row] = data.get_y(row);
    }

    for (iteration = 0; iteration < data.iterations; iteration++) {
        var y_fit = Array(data.rows);

        for (row = 0; row < data.rows; row++) {
            y_fit[row] = data.get_y_fit(iteration, row);
        }
    ...
    Plotly.newPlot(
        plot_id,
        {
            ...
            layout: {
                ...set_plot_size(plot_id, width_mult=1, height_mult=0.6),
                margin: {
                    l: 20,
                    r: 20,
                    b: 20,
...
function plot3d(plot_id, data) {
    ...
    var x1 = Array(data.rows);
    var x2 = Array(data.rows);
    var y = Array(data.rows);
    for (row = 0; row < data.rows; row++) {
        x1[row] = data.get_x(row, 0);
        x2[row] = data.get_x(row, 1);
        y[row] = data.get_y(row);
    }
    ...
    for (iteration = 0; iteration < data.iterations; iteration++) {
        ...
        var z_iso = Array(data.rows);
        for (row = 0; row < data.rows; row++) {
            z_iso[row] = data.get_y_fit(iteration, row);
        }
...
function plot_result() {
    ...
    if (result.cols == 1) {
        plot2d("IsoPlot", result)
    } else if (result.cols == 2) {
        plot3d("IsoPlot", result)
    } else {
        alert("plotting is only supported in 2 and 3 dimensions!")
        return false
    }
}
...
function set_console_element() {
    if (!runtimeInitialized) {
        setTimeout(set_console_element, 50)
    } else {
...
function init(){
    ...
    document.getElementById('example-1').onclick = set_example;
    document.getElementById('example-2').onclick = function() {
        document.getElementById('input').value = Module.generate_input_data(40, 1);
    }
    document.getElementById('example-3').onclick = function() {
        document.getElementById('input').value = Module.generate_input_data(40, 2);
    }
    set_console_element();
}
